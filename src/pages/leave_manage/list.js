...
export default function List() {
          ...
          return (
            <Button.Group>
              <Button
                size="xs"
                color="blue"
                onClick={() => router.push(`/leave/edit/${encryptedId}`)}
  ...
  const handleDelete = async (encryptedId) => {
    ...
    try {
      ...
      await showAlert(
        "Success",
        "success",
        "Employee Leave deleted successfully",
        false,
        1500
      );
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data } = await axios.post(
      API_URL +
        `/api/leave/serverside?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
      {},
      {
        headers: {
        ...
        <div className="max-w-full mx-auto sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="px-4 py-3 border-b flex items-center gap-2">
              <IconList size={20} />
              <h2 className="text-lg font-semibold">List Leave Employee</h2>
            </div>
            <div className="px-4 py-2 text-right space-x-2">
              {/* <Button onClick={() => router.push("/leave/create")}>
                {" "}
                Add Employee
              </Button> */}
