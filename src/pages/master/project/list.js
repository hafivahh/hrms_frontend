...
export default function List() {
  ...
  const canCreate = hasPermission(34);
  const canUpdate = hasPermission(35);
  const canDelete = hasPermission(36);
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data } = await axios.post(
      API_URL +
        `/api/master/project/serverside?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${user.token}`,
