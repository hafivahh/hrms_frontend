...
import {
  ...
  Checkbox, Collapse, Divider, Loader, Select,
} from "@mantine/core";
...
export default function Create_Role() {
  ...
  // fetch roles untuk Copy Permission From
  useEffect(() => {
    if (!user?.token) return;
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${API_URL}/api/master/role/create`,
        { role_name: values.role_name },
        { headers: { Authorization: `Bearer ${user.token}` } },
      );

      if (!data.success) {
        await showAlert("Error", "error", data.message || "Failed to create role");
        return;
      }

      const newRoleId = data.data?.id_role;

      // Step 2: save permissions jika ada
      if (newRoleId && selectedPermissions.length > 0) {
        await axios.post(
          `${API_URL}/api/master/role/permissions/${newRoleId}`,
          {
      ...
      await showAlert("Success", "success", data.message || "Role created successfully", false, 1500);
      router.push("/master/role/list");
    } catch (error) {
      const err = error.response?.data || {};
      showAlert(err.message || "Error", "error", err.error || "Failed to create role");
    } finally {
      setLoading(false);
    }
  };
