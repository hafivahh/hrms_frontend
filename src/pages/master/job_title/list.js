...
export default function List() {
                ...
                return (
                  <Button.Group>
                    <Button
                      ...
                      onClick={() =>
                        router.push(
                          `/master/job_title/edit/${encryptedId}`
                        )
                      }
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data } = await axios.post(
      API_URL +
        `/api/master/jobtitle/serverside?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
      {},
      {
        headers: {
      ...
      <div className="py-6">
        <div className="max-w-full mx-auto sm:px-6 lg:px-8">
          <Paper
            radius="sm"
            mt="md"
            style={{ job_title: "relative" }}
            withBorder
          >
            {/* JUDUL + ICON */}
            <div className="px-4 py-3 border-b flex items-center gap-2">
              <IconDatabase size={20} />
              <h2 className="text-lg font-semibold">Master Job Title</h2>
            </div>

            {/* Tombol di kanan */}
            <div className="px-4 py-2 text-right space-x-2">
              <Button onClick={() => router.push("/master/job_title/create")}>
                Add Job Title
              </Button>
