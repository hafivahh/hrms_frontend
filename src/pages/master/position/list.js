...
export default function List() {
  ...
  const canCreate = hasPermission(42);
  const canUpdate = hasPermission(43);
  const canDelete = hasPermission(44);
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data } = await axios.post(
      API_URL +
        `/api/master/position/serverside?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${user.token}`,
        ...
        <div className="max-w-full mx-auto sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="px-4 py-3 border-b flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold">Master Position</h2>
            </div>
