...
export default function IssMprCreate() {
  ...
  const validateForm = () => {
    ...
    if (!formData.experience_years)
      e.experience_years = "Years of Experience is required";
    if (!formData.education_level.length)
      e.education_level = "At least one Education is required";
    ...
    if (!formData.acknowledged_by)
      e.acknowledged_by = "Acknowledged By is required";
    ...
    try {
      ...
      if (ok) {
        await showAlert(
          "Success",
          "success",
          data?.message || "MPR created successfully",
          false,
          1500,
        );
        router.back();
      ...
      if (err)
        showAlert(
          err.message || "Failed to create MPR",
          "error",
          err.error || "",
        );
          ...
          <Paper radius="sm" mt="md" withBorder>
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                {errors.budgeted && (
                  <p className="text-red-500 text-xs mt-1">{errors.budgeted}</p>
                )}
              </div>
                    ...
                    <Table.Tr key={field}>
                      ...
                      <Table.Td>
                        <ManagerSelect
                          ...
                          onChange={(val) =>
                            handleInputChange(field, normalizeUserId(val))
                          }
                        />
                        {errors[field] && (
                          <p className="text-red-500 text-xs mt-1">
